 ...
 * (migration 202608160001). The resolved aggregate output budget can never exceed
...
const WORST_CASE_SCRIPT_CALLS = 4; // 2 GENERATOR (draft) + 2 REVISION
...
export function channelVideoScriptConfig() {
  ...
  if ((process.env.VIDEO_SCRIPT_MODEL_MAX_OUTPUT_TOKENS ?? "").trim() !== "") {
    throw new Error("VIDEO_SCRIPT_MODEL_MAX_OUTPUT_TOKENS is no longer supported; set VIDEO_SCRIPT_SCRIPT_OUTPUT_TOKENS (GENERATOR/REVISION) and VIDEO_SCRIPT_REVIEW_OUTPUT_TOKENS (CRITIC/QA) instead.");
  }
  ...
  // 18,000 = 216,000 tokens, far above the 100,000 database maximum. The two roles
  // that emit a whole script (GENERATOR draft, REVISION) get the large allowance;
  // the review roles (CRITIC, QA) emit only structured findings and get a much
  // smaller one. See video-script-model.ts (reservation is role-keyed) and
  // video-script-budget.test.ts (walks the worst-case sequence).
  const modelScriptOutputTokens = boundedInteger("VIDEO_SCRIPT_SCRIPT_OUTPUT_TOKENS", 15_000, 2_000, 20_000);
  const modelReviewOutputTokens = boundedInteger("VIDEO_SCRIPT_REVIEW_OUTPUT_TOKENS", 4_000, 1_000, 8_000);
  const maxAggregateInputTokens = boundedInteger("VIDEO_SCRIPT_MAX_AGGREGATE_INPUT_TOKENS", 700_000, 20_000, 1_000_000);
  const maxAggregateOutputTokens = boundedInteger("VIDEO_SCRIPT_MAX_AGGREGATE_OUTPUT_TOKENS", 96_000, 4_000, 100_000);
  const maxAggregateTotalTokens = boundedInteger("VIDEO_SCRIPT_MAX_AGGREGATE_TOTAL_TOKENS", 796_000, 24_000, 1_100_000);
  ...
  // 12-call retry graph reserves WORST_CASE_SCRIPT_CALLS script ceilings plus
  ...
  const worstCaseOutput = WORST_CASE_SCRIPT_CALLS * modelScriptOutputTokens + WORST_CASE_REVIEW_CALLS * modelReviewOutputTokens;
  if (maxAggregateOutputTokens > DATABASE_OUTPUT_CEILING) {
    throw new Error(`VIDEO_SCRIPT budget invalid: maxAggregateOutputTokens ${maxAggregateOutputTokens} exceeds the ${DATABASE_OUTPUT_CEILING} database output ceiling.`);
  }
  if (worstCaseOutput > maxAggregateOutputTokens) {
    throw new Error(`VIDEO_SCRIPT budget invalid: the full retry graph reserves ${worstCaseOutput} output tokens (${WORST_CASE_SCRIPT_CALLS}*${modelScriptOutputTokens} + ${WORST_CASE_REVIEW_CALLS}*${modelReviewOutputTokens}) but maxAggregateOutputTokens is ${maxAggregateOutputTokens}. Lower the role ceilings or raise VIDEO_SCRIPT_MAX_AGGREGATE_OUTPUT_TOKENS (<= ${DATABASE_OUTPUT_CEILING}).`);
  }
  ...
  if (maxAggregateTotalTokens < maxAggregateInputTokens + maxAggregateOutputTokens) {
    throw new Error(`VIDEO_SCRIPT budget invalid: maxAggregateTotalTokens ${maxAggregateTotalTokens} is below maxAggregateInputTokens + maxAggregateOutputTokens (${maxAggregateInputTokens + maxAggregateOutputTokens}).`);
  }
