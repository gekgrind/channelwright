-- CHANNEL_VIDEO_BRIEF vertical slice. Additive and forward-only: it extends the
-- existing workflow, approval, lease, lineage, immutability, and usage-accounting
-- spine rather than introducing a parallel mechanism. No previously applied
-- migration is altered.
--
-- Nothing here widens the database resource ceilings introduced in 202608130004.
-- This workflow performs no external evidence retrieval at all: it reasons over
-- the discovery bundle the approved content-intelligence run already paid for,
-- so its provider-request, quota, and search ceilings are zero and only model
-- calls consume budget.
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF'));

-- One active video-brief run per owner per approved content run per selected
-- topic. Two different topics from the same approved backlog may legitimately
-- be briefed concurrently, so the topic is part of the key. BLOCKED is excluded
-- so a human-revision successor (predecessor moved to BLOCKED before the
-- successor is inserted) remains legal.
create unique index workflow_runs_active_video_brief_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedContentReference'->>'contentRunId')),
    ((input_payload->>'selectedTopicId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_BRIEF' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');

-- ---------------------------------------------------------------------------
-- Approved content-intelligence resolver
--
-- Resolves the exact approved artifact and the one eligible topic the brief is
-- for. Cross-owner requests are NOT_FOUND rather than FORBIDDEN so the ID space
-- stays non-enumerable. When p_topic_id is null the artifact's own authoritative
-- nextVideoRecommendation is used; a supplied topic must be in the approved
-- ranked backlog, belong to this exact artifact, and pass the Viewer Value gate.
-- ---------------------------------------------------------------------------
create or replace function channelwright.resolve_approved_content_artifact(
  p_workflow_id uuid,
  p_run_id uuid,
...
begin
  select * into v_run from channelwright.workflow_runs
    where id = p_run_id and workflow_id = p_workflow_id and workflow_type = 'CHANNEL_CONTENT_INTELLIGENCE';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_CONTENT_INTELLIGENCE run'; end if;
  select * into v_workflow from channelwright.workflows
    where id = p_workflow_id and owner_id = v_run.owner_id and workflow_type = 'CHANNEL_CONTENT_INTELLIGENCE';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_CONTENT_INTELLIGENCE workflow'; end if;
  if v_role <> 'service_role' and (auth.uid() is null or auth.uid() <> v_run.owner_id) then
    raise exception 'NOT_FOUND: exact CHANNEL_CONTENT_INTELLIGENCE run';
  end if;

  if v_run.status <> 'COMPLETED' or v_run.output_payload is null then
    raise exception 'UPSTREAM_CONTENT_NOT_FINAL: content intelligence must be completed and final';
  end if;
  ...
  if not found then raise exception 'UPSTREAM_CONTENT_NOT_APPROVED: exact content-intelligence run is not human approved'; end if;

  select output_payload into v_finalizer from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'finalize-content-intelligence' and status = 'COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'final-content-qa' and status = 'COMPLETED';
  -- discover-youtube-topics is the provenance step for content intelligence: it
  -- carries the discovery evidence bundle every downstream claim must cite.
  select output_payload into v_provenance from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'discover-youtube-topics' and status = 'COMPLETED';
  select output_payload into v_upstream_strategy from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'validate-approved-strategy' and status = 'COMPLETED';

  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then
    raise exception 'UPSTREAM_CONTENT_INTEGRITY_MISMATCH: final output disagreement';
  end if;
  ...
  then raise exception 'UPSTREAM_CONTENT_QA_INVALID: final QA did not pass'; end if;
  if v_provenance is null or jsonb_typeof(v_provenance->'evidence') <> 'array'
    or jsonb_array_length(v_provenance->'evidence') < 1
  then raise exception 'UPSTREAM_CONTENT_PROVENANCE_INVALID: discovery evidence is missing'; end if;
  if v_upstream_strategy is null or v_upstream_strategy->'reference' is null then
    raise exception 'UPSTREAM_CONTENT_PROVENANCE_INVALID: upstream strategy provenance is missing';
  end if;
  -- The content artifact must still agree with the strategy reference it was
  -- built on, which itself already carries the approved research reference.
  if v_run.output_payload->'upstreamStrategy' is distinct from v_upstream_strategy->'reference' then
    raise exception 'UPSTREAM_CONTENT_INTEGRITY_MISMATCH: content intelligence no longer agrees with its upstream strategy reference';
  end if;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null
    ...
    raise exception 'UPSTREAM_CONTENT_INTEGRITY_MISMATCH: stored integrity data does not match';
  end if;

  begin v_parent := nullif(v_run.context_payload->>'previousRunId','')::uuid;
  exception when invalid_text_representation then raise exception 'UPSTREAM_CONTENT_LINEAGE_INVALID'; end;
  ...
  -- Topic selection: default to the artifact's own authoritative recommendation.
  if p_topic_id is null or btrim(p_topic_id) = '' then
    v_topic_id := v_run.output_payload->'nextVideoRecommendation'->>'topicId';
    v_selection_source := 'NEXT_VIDEO_RECOMMENDATION';
    if v_topic_id is null then raise exception 'UPSTREAM_CONTENT_INTEGRITY_MISMATCH: artifact carries no next-video recommendation'; end if;
  else
    v_topic_id := btrim(p_topic_id);
    if v_topic_id !~ '^topic:[a-z0-9][a-z0-9-]{0,58}$' then raise exception 'VALIDATION_ERROR: malformed topic identifier'; end if;
    v_selection_source := case when v_topic_id = v_run.output_payload->'nextVideoRecommendation'->>'topicId'
      then 'NEXT_VIDEO_RECOMMENDATION' else 'OPERATOR_SELECTED' end;
  end if;

  -- The topic must be in this exact artifact's ranked backlog...
  select entry into v_backlog from jsonb_array_elements(v_run.output_payload->'backlog') as t(entry)
    where entry->>'topicId' = v_topic_id;
  if v_backlog is null then raise exception 'TOPIC_NOT_IN_APPROVED_BACKLOG: the selected topic is not in the approved ranked backlog'; end if;

  -- ...and must be one of its own topic opportunities.
  select topic into v_topic from jsonb_array_elements(v_run.output_payload->'topics') as t(topic)
    where topic->>'topicId' = v_topic_id;
  if v_topic is null then raise exception 'TOPIC_NOT_IN_APPROVED_ARTIFACT: the selected topic does not belong to this content-intelligence artifact'; end if;

  -- Viewer Value eligibility. A topic that did not pass its own gate upstream is
  -- not eligible to become a video brief.
  v_gate := v_topic->'viewerValue'->>'gate';
  if v_gate is distinct from 'PASS' then
    raise exception 'TOPIC_VIEWER_VALUE_NOT_ELIGIBLE: the selected topic did not pass the Viewer Value gate';
  end if;
  if jsonb_typeof(v_topic->'evidenceIds') <> 'array' or jsonb_array_length(v_topic->'evidenceIds') < 1 then
    raise exception 'TOPIC_EVIDENCE_MISSING: the selected topic cites no discovery evidence';
  end if;

  return jsonb_build_object(
    'reference', jsonb_build_object(
      'contentWorkflowId', v_workflow.id,
      'contentRunId', v_run.id,
      ...
      'contentArtifactHash', v_artifact_hash,
      'contentProvenanceHash', v_provenance_hash,
      'parentRunId', v_parent,
      'rootRunId', v_root,
      'upstreamStrategy', v_upstream_strategy->'reference'
    ),
    'contentResult', v_run.output_payload,
    'discoveryBundle', v_provenance,
    'selectedTopic', v_topic,
    'selection', jsonb_build_object(
      'topicId', v_topic_id,
      'pillarId', v_topic->>'pillarId',
      'backlogRank', (v_backlog->>'rank')::integer,
      'tier', v_backlog->>'tier',
      'selectionSource', v_selection_source,
      ...
      -- topic's own viewer-value contract, so a downstream stage that quietly
      ...
      'inheritedViewerValueProvenance', jsonb_build_object(
        ...
        'contractHash', encode(digest(channelwright.canonical_jsonb_text(v_topic->'viewerValue'->'contract'), 'sha256'), 'hex'),
        'gate', v_gate,
...
-- ---------------------------------------------------------------------------
-- Canonical JSON text, so a contract hash is key-order independent and agrees
-- with the application's `canonicalJson`. Object keys are sorted recursively;
-- array order is meaningful and preserved.
-- ---------------------------------------------------------------------------
create or replace function channelwright.canonical_jsonb_text(p_value jsonb)
returns text language sql immutable set search_path = channelwright, pg_temp as $$
  select case jsonb_typeof(p_value)
    when 'object' then coalesce((
      -- `collate "C"` is load-bearing: the application sorts keys by code unit,
      -- while a bare `order by key` would use the database collation. Under
      -- en_US.UTF-8 an uppercase letter sorts case-insensitively, so sibling
      -- camelCase keys such as originStage and originalContribution would order
      -- differently in the two implementations and a semantically identical
      -- contract would fail provenance verification.
      select '{' || string_agg(to_json(key)::text || ':' || channelwright.canonical_jsonb_text(value), ',' order by key collate "C") || '}'
      from jsonb_each(p_value)
    ), '{}')
    when 'array' then coalesce((
      select '[' || string_agg(channelwright.canonical_jsonb_text(value), ',' order by ordinality) || ']'
      from jsonb_array_elements(p_value) with ordinality as t(value, ordinality)
    ), '[]')
    -- jsonb keeps numeric scale, so 1.50 would render as "1.50" while
    -- JSON.stringify emits "1.5". trim_scale normalizes that away.
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
  ...
  if p_workflow_type = 'CHANNEL_RESEARCH' then
    ...
    -- Only identifiers and one bounded selector are accepted. The browser can
    -- never submit a topic object, a content result, a provenance hash, or any
    -- QA state: those are resolved from authoritative database state below.
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the fourth paid workflow type.
--
-- Reproduced verbatim from 202608140003 apart from the added CHANNEL_VIDEO_BRIEF
-- finalizer case: the service-role guard, idempotent-replay path, lease and
-- attempt checks, and the context_payload write that feeds priorOutputs are all
-- load-bearing for every workflow and must not drift.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Approval, revision lineage, and provenance hashing extend to the video brief.
-- The video brief's provenance step is validate-approved-content: it carries the
-- exact upstream artifact, the resolved topic, and the discovery bundle every
-- downstream claim must cite. A human-requested revision preserves the original
-- run, the pinned upstream reference, the selected topic, and root lineage.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF') and v_old_run.output_payload is not null then
  ...
  if p_decision='REJECT' then
    ...
    insert into channelwright.workflow_events(owner_id,workflow_id,workflow_run_id,event_type,actor_type,actor_id,detail)
      values(v_owner,p_workflow_id,v_new_run_id,'WORKFLOW_REVISION_QUEUED','USER',v_owner::text,jsonb_build_object('previousRunId',v_old_run.id,'upstreamResearchRunId',v_old_run.input_payload->'approvedResearchReference'->>'researchRunId','upstreamStrategyRunId',v_old_run.input_payload->'approvedStrategyReference'->>'strategyRunId','upstreamContentRunId',v_old_run.input_payload->'approvedContentReference'->>'contentRunId','selectedTopicId',v_old_run.input_payload->>'selectedTopicId'));
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the fourth paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_BRIEF joins CHANNEL_STRATEGY as a workflow that performs no
-- external retrieval, so it is required to declare exactly zero provider
-- requests, quota units, and searches. Model-call and token ceilings are
-- unchanged from 202608130004.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF');
...
revoke all on function channelwright.resolve_approved_content_artifact(uuid,uuid,text) from public,anon;
grant execute on function channelwright.resolve_approved_content_artifact(uuid,uuid,text) to authenticated,service_role;
revoke all on function channelwright.canonical_jsonb_text(jsonb) from public,anon;
grant execute on function channelwright.canonical_jsonb_text(jsonb) to authenticated,service_role;
