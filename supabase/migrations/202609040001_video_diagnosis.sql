-- CHANNEL_VIDEO_DIAGNOSIS vertical slice. Additive and forward-only: it extends
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS'));
...
create unique index if not exists workflow_runs_active_video_performance_uniq
...
create unique index workflow_runs_active_video_diagnosis_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedVideoPerformanceReference'->>'performanceRunId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_DIAGNOSIS' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS') and v_old_run.output_payload is not null then
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS');
  if jsonb_object_length(p_limits)<>10
    ...
    or channelwright.research_usage_value(p_limits,'synthesisCalls') not between 1 and (case when v_run.workflow_type='CHANNEL_VIDEO_DIAGNOSIS' then 2 else 6 end)
    or channelwright.research_usage_value(p_limits,'qaCalls') not between 1 and (case when v_run.workflow_type='CHANNEL_VIDEO_DIAGNOSIS' then 2 else 12 end)
    or channelwright.research_usage_value(p_limits,'revisionCalls') not between (case when v_run.workflow_type='CHANNEL_VIDEO_DIAGNOSIS' then 0 else 1 end) and (case when v_run.workflow_type='CHANNEL_VIDEO_DIAGNOSIS' then 0 else 4 end)
    ...
    or channelwright.research_usage_value(p_limits,'automatedRevisions')<>(case when v_run.workflow_type='CHANNEL_VIDEO_DIAGNOSIS' then 0 else 1 end)
...
-- Exact approved-Performance resolution for CHANNEL_VIDEO_DIAGNOSIS. The RPC is
-- the sole authority for upstream identity and compact transitive lineage.
create or replace function channelwright.resolve_approved_video_performance_artifact(p_workflow_id uuid,p_run_id uuid)
returns jsonb language plpgsql security definer set search_path=channelwright,extensions,pg_temp as $$
declare
  ...
  v_release_ref jsonb; v_packaging_ref jsonb; v_script_ref jsonb; v_brief_ref jsonb; v_content_ref jsonb; v_strategy_ref jsonb; v_research_ref jsonb;
  v_release jsonb; v_packaging jsonb; v_script jsonb; v_brief jsonb; v_content jsonb; v_strategy jsonb; v_research jsonb;
  v_artifacts jsonb; v_entries jsonb:='[]'::jsonb; v_facts jsonb; v_link jsonb; v_chain_run channelwright.workflow_runs%rowtype;
  v_role text:=coalesce(auth.jwt()->>'role','');
begin
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_id=p_workflow_id and workflow_type='CHANNEL_VIDEO_PERFORMANCE';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_PERFORMANCE run'; end if;
  select * into v_workflow from channelwright.workflows where id=p_workflow_id and owner_id=v_run.owner_id and workflow_type='CHANNEL_VIDEO_PERFORMANCE';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_PERFORMANCE workflow'; end if;
  if v_role<>'service_role' and (auth.uid() is null or auth.uid()<>v_run.owner_id) then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_PERFORMANCE run'; end if;
  if v_run.status<>'COMPLETED' or v_run.output_payload is null then raise exception 'UPSTREAM_PERFORMANCE_NOT_FINAL: Performance must be completed and final'; end if;
  if v_workflow.current_run_id is distinct from v_run.id or exists(select 1 from channelwright.workflow_runs r where r.workflow_id=v_run.workflow_id and r.owner_id=v_run.owner_id and r.context_payload->>'previousRunId'=v_run.id::text)
    then raise exception 'UPSTREAM_PERFORMANCE_SUPERSEDED: Performance is not the latest immutable successor'; end if;
  ...
  if not found then raise exception 'UPSTREAM_PERFORMANCE_NOT_APPROVED: exact Performance run is not human approved'; end if;
  select output_payload into v_finalizer from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='finalize-video-performance' and status='COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='final-video-performance-qa' and status='COMPLETED';
  select output_payload into v_provenance from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='validate-approved-release' and status='COMPLETED';
  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then raise exception 'UPSTREAM_PERFORMANCE_INTEGRITY_MISMATCH: finalizer output disagreement'; end if;
  if v_final_qa is null or coalesce((v_final_qa->'qa'->>'passed')::boolean,false) is not true or v_final_qa->'qa'->>'recommendation' not in ('accept','human_review_required') or coalesce((v_final_qa->'qa'->>'score')::integer,-1) not between 0 and 100
    then raise exception 'UPSTREAM_PERFORMANCE_QA_INVALID: final QA did not pass'; end if;
  if v_provenance is null or v_provenance->'reference' is null or v_provenance->'releaseResult' is null then raise exception 'UPSTREAM_PERFORMANCE_PROVENANCE_INVALID: authoritative Release provenance is missing'; end if;
  if v_run.output_payload->'upstreamVideoRelease' is distinct from v_provenance->'reference' then raise exception 'UPSTREAM_PERFORMANCE_INTEGRITY_MISMATCH: nested Release reference drift'; end if;
  if v_run.output_payload->'measuredSnapshot' is distinct from v_run.input_payload->'performanceSnapshot' then raise exception 'UPSTREAM_PERFORMANCE_SNAPSHOT_MISMATCH: immutable measured snapshot drift'; end if;
  begin
    if (v_run.output_payload#>>'{measuredSnapshot,observationWindow,start}')::timestamptz>(v_run.output_payload#>>'{measuredSnapshot,observationWindow,end}')::timestamptz or (v_run.output_payload#>>'{measuredSnapshot,observationWindow,end}')::timestamptz>(v_run.output_payload#>>'{measuredSnapshot,capturedAt}')::timestamptz
      then raise exception 'UPSTREAM_PERFORMANCE_TIMESTAMP_INVALID: inconsistent snapshot timestamps'; end if;
  exception when invalid_datetime_format then raise exception 'UPSTREAM_PERFORMANCE_TIMESTAMP_INVALID: malformed snapshot timestamps'; end;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null or v_run.artifact_hash<>v_artifact_hash or v_run.provenance_hash<>v_provenance_hash then raise exception 'UPSTREAM_PERFORMANCE_INTEGRITY_MISMATCH: canonical hash mismatch'; end if;
  begin v_parent:=nullif(v_run.context_payload->>'previousRunId','')::uuid; exception when invalid_text_representation then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: malformed parent'; end;
  ...
  if v_parent is not null and not exists(select 1 from channelwright.workflow_runs where id=v_parent and workflow_id=v_run.workflow_id and owner_id=v_run.owner_id and workflow_type=v_run.workflow_type) then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: parent/root drift'; end if;
  v_release_ref:=v_run.output_payload->'upstreamVideoRelease'; v_packaging_ref:=v_release_ref->'upstreamVideoPackaging'; v_script_ref:=v_packaging_ref->'upstreamVideoScript'; v_brief_ref:=v_script_ref->'upstreamVideoBrief'; v_content_ref:=v_brief_ref->'upstreamContentIntelligence'; v_strategy_ref:=v_content_ref->'upstreamStrategy'; v_research_ref:=v_strategy_ref->'upstreamResearch';
  if v_release_ref is null or v_packaging_ref is null or v_script_ref is null or v_brief_ref is null or v_content_ref is null or v_strategy_ref is null or v_research_ref is null then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: transitive reference missing'; end if;
  v_artifacts:=jsonb_build_array(
    jsonb_build_object('workflowType','CHANNEL_RESEARCH','runId',v_research_ref->>'researchRunId','artifactHash',v_research_ref->>'researchArtifactHash','schemaVersion',(v_research_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_STRATEGY','runId',v_strategy_ref->>'strategyRunId','artifactHash',v_strategy_ref->>'strategyArtifactHash','schemaVersion',(v_strategy_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_CONTENT_INTELLIGENCE','runId',v_content_ref->>'contentRunId','artifactHash',v_content_ref->>'contentArtifactHash','schemaVersion',(v_content_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_VIDEO_BRIEF','runId',v_brief_ref->>'briefRunId','artifactHash',v_brief_ref->>'briefArtifactHash','schemaVersion',(v_brief_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_VIDEO_SCRIPT','runId',v_script_ref->>'scriptRunId','artifactHash',v_script_ref->>'scriptArtifactHash','schemaVersion',(v_script_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_VIDEO_PACKAGING','runId',v_packaging_ref->>'packagingRunId','artifactHash',v_packaging_ref->>'packagingArtifactHash','schemaVersion',(v_packaging_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_VIDEO_RELEASE','runId',v_release_ref->>'releaseRunId','artifactHash',v_release_ref->>'releaseArtifactHash','schemaVersion',(v_release_ref->>'outputSchemaVersion')::integer),
    jsonb_build_object('workflowType','CHANNEL_VIDEO_PERFORMANCE','runId',v_run.id,'artifactHash',v_artifact_hash,'schemaVersion',(v_run.output_payload->>'schemaVersion')::integer));
  for v_link in select value from jsonb_array_elements(v_artifacts) loop
    ...
    if not found or v_chain_run.status<>'COMPLETED' or v_chain_run.output_payload is null or encode(digest(v_chain_run.output_payload::text,'sha256'),'hex')<>v_link->>'artifactHash' or (v_chain_run.output_payload->>'schemaVersion')::integer<>(v_link->>'schemaVersion')::integer then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: transitive artifact mismatch'; end if;
    if v_link->>'workflowType'='CHANNEL_RESEARCH' then v_research:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_STRATEGY' then v_strategy:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_CONTENT_INTELLIGENCE' then v_content:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_VIDEO_BRIEF' then v_brief:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_VIDEO_SCRIPT' then v_script:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_VIDEO_PACKAGING' then v_packaging:=v_chain_run.output_payload; elsif v_link->>'workflowType'='CHANNEL_VIDEO_RELEASE' then v_release:=v_chain_run.output_payload; end if;
  end loop;
  if v_release is distinct from v_provenance->'releaseResult' then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: Release snapshot contradiction'; end if;
  if exists(select 1 from jsonb_array_elements(v_packaging->'titleCandidates') x group by x->>'candidateId' having count(*)>1) or exists(select 1 from jsonb_array_elements(v_packaging->'thumbnailConcepts') x group by x->>'conceptId' having count(*)>1) or exists(select 1 from jsonb_array_elements(v_packaging->'chapters') x group by x->>'chapterId' having count(*)>1) or exists(select 1 from jsonb_array_elements(v_script->'sections') x group by x->>'sectionId' having count(*)>1) or exists(select 1 from jsonb_array_elements(v_brief#>'{contentArchitecture,beats}') x group by x->>'sectionId' having count(*)>1) or exists(select 1 from jsonb_array_elements(v_brief#>'{evidencePlan,items}') x group by x->>'claimId' having count(*)>1) then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: duplicate stable identity'; end if;
  v_entries:=jsonb_build_array(
    jsonb_build_object('key','lin:pillar:'||(v_run.output_payload#>>'{source,pillarId}'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','PILLAR','id',v_run.output_payload#>>'{source,pillarId}'),'parentKeys','[]'::jsonb,'label',v_run.output_payload#>>'{source,pillarName}'),
    jsonb_build_object('key','lin:topic:'||(v_run.output_payload#>>'{source,releaseTopicId}'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','TOPIC','id',v_run.output_payload#>>'{source,releaseTopicId}'),'parentKeys',jsonb_build_array('lin:pillar:'||(v_run.output_payload#>>'{source,pillarId}')),'label',v_run.output_payload#>>'{source,workingConcept}'));
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:title:'||(x.value->>'candidateId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','TITLE_CANDIDATE','id',x.value->>'candidateId'),'parentKeys',jsonb_build_array('lin:topic:'||(v_run.output_payload#>>'{source,releaseTopicId}')),'label',x.value->>'text') order by x.ord) from jsonb_array_elements(v_packaging->'titleCandidates') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:thumbnail:'||(x.value->>'conceptId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','THUMBNAIL_CONCEPT','id',x.value->>'conceptId'),'parentKeys',jsonb_build_array('lin:topic:'||(v_run.output_payload#>>'{source,releaseTopicId}')),'label',coalesce(x.value->>'copyText',x.value->>'visualIntent')) order by x.ord) from jsonb_array_elements(v_packaging->'thumbnailConcepts') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:chapter:'||(x.value->>'chapterId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','CHAPTER','id',x.value->>'chapterId'),'parentKeys',jsonb_build_array('lin:script-section:'||(x.value->>'sourceScriptSectionId')),'label',x.value->>'title') order by x.ord) from jsonb_array_elements(v_packaging->'chapters') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:script-section:'||(x.value->>'sectionId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','SCRIPT_SECTION','id',x.value->>'sectionId'),'parentKeys',jsonb_build_array('lin:brief-beat:'||(x.value->>'briefBeatId')),'label',x.value->>'title') order by x.ord) from jsonb_array_elements(v_script->'sections') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:brief-beat:'||(x.value->>'sectionId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','BRIEF_BEAT','id',x.value->>'sectionId'),'parentKeys',jsonb_build_array('lin:topic:'||(v_run.output_payload#>>'{source,releaseTopicId}')),'label',x.value->>'title') order by x.ord) from jsonb_array_elements(v_brief#>'{contentArchitecture,beats}') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:evidence:'||(x.value->>'id'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','EVIDENCE','id',x.value->>'id'),'parentKeys','[]'::jsonb,'label',coalesce(x.value->>'title',x.value->>'query')) order by x.ord) from jsonb_array_elements(v_provenance#>'{discoveryBundle,evidence}') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:brief-claim:'||(x.value->>'claimId'),'locator',jsonb_build_object('kind','STABLE_ID','entityType','BRIEF_CLAIM','id',x.value->>'claimId'),'parentKeys',coalesce((select jsonb_agg('lin:evidence:'||e.value) from jsonb_array_elements_text(x.value->'evidenceIds') e(value)),'[]'::jsonb),'label',x.value->>'claim') order by x.ord) from jsonb_array_elements(v_brief#>'{evidencePlan,items}') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:strategy-pillar:'||(x.ord-1),'locator',jsonb_build_object('kind','ARTIFACT_LOCAL','entityType','STRATEGY_PILLAR','workflowType','CHANNEL_STRATEGY','runId',v_strategy_ref->>'strategyRunId','artifactHash',v_strategy_ref->>'strategyArtifactHash','schemaVersion',(v_strategy_ref->>'outputSchemaVersion')::integer,'jsonPointer','/contentPillars/'||(x.ord-1)),'parentKeys','[]'::jsonb,'label',x.value->>'name') order by x.ord) from jsonb_array_elements(v_strategy->'contentPillars') with ordinality x(value,ord)),'[]'::jsonb);
  v_entries:=v_entries||coalesce((select jsonb_agg(jsonb_build_object('key','lin:strategy-kpi:'||(x.ord-1),'locator',jsonb_build_object('kind','ARTIFACT_LOCAL','entityType','STRATEGY_KPI','workflowType','CHANNEL_STRATEGY','runId',v_strategy_ref->>'strategyRunId','artifactHash',v_strategy_ref->>'strategyArtifactHash','schemaVersion',(v_strategy_ref->>'outputSchemaVersion')::integer,'jsonPointer','/kpiFramework/'||(x.ord-1)),'parentKeys','[]'::jsonb,'label',x.value->>'label') order by x.ord) from jsonb_array_elements(v_strategy->'kpiFramework') with ordinality x(value,ord)),'[]'::jsonb);
  if exists(select 1 from jsonb_array_elements(v_entries) x group by x->>'key' having count(*)>1) then raise exception 'UPSTREAM_PERFORMANCE_LINEAGE_INVALID: duplicate projected key'; end if;
  v_facts:=jsonb_build_array(
    jsonb_build_object('key','fact:final-title','value',v_run.output_payload#>'{source,finalTitle}','sourceRef','performance:/source/finalTitle'),
    jsonb_build_object('key','fact:selected-title-candidate','value',v_release#>'{titleDecision,selectedCandidateId}','sourceRef','release:/titleDecision/selectedCandidateId'),
    jsonb_build_object('key','fact:release-promise','value',v_run.output_payload#>'{source,releasePromise}','sourceRef','performance:/source/releasePromise'),
    jsonb_build_object('key','fact:opening-spoken','value',v_script#>'{openingHook,spokenOpening}','sourceRef','script:/openingHook/spokenOpening'),
    jsonb_build_object('key','fact:selected-thumbnail-concept','value',v_release#>'{thumbnailDecision,selectedConceptId}','sourceRef','release:/thumbnailDecision/selectedConceptId'),
    jsonb_build_object('key','fact:snapshot-coverage','value',v_run.output_payload#>'{snapshotIntegrity,coverage}','sourceRef','performance:/snapshotIntegrity/coverage'));
  return jsonb_build_object('reference',jsonb_build_object(
    'performanceWorkflowId',v_workflow.id,'performanceRunId',v_run.id,'workflowDefinitionVersion',v_run.definition_version,'outputSchemaVersion',(v_run.output_payload->>'schemaVersion')::integer,
    ...
    'finalQaState',v_final_qa->'qa'->>'recommendation','finalQaScore',(v_final_qa->'qa'->>'score')::integer,'performanceArtifactHash',v_artifact_hash,'performanceProvenanceHash',v_provenance_hash,
    'parentRunId',v_parent,'rootRunId',v_root,'upstreamVideoRelease',v_release_ref),
    'performanceResult',v_run.output_payload,'diagnosisScope',jsonb_build_object('artifacts',v_artifacts,'entries',v_entries,'facts',v_facts));
end $$;

revoke all on function channelwright.resolve_approved_video_performance_artifact(uuid,uuid) from public,anon;
grant execute on function channelwright.resolve_approved_video_performance_artifact(uuid,uuid) to authenticated,service_role;
