-- CHANNEL_VIDEO_SCRIPT vertical slice. Additive and forward-only: it extends the
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT'));

-- One active video-script run per owner per approved video-brief run. A brief
-- pins one topic, so the brief run alone is the key. BLOCKED is excluded so a
-- human-revision successor (predecessor moved to BLOCKED before the successor is
-- inserted) remains legal.
create unique index workflow_runs_active_video_script_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedVideoBriefReference'->>'briefRunId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_SCRIPT' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');

-- ---------------------------------------------------------------------------
-- Approved video-brief resolver
--
-- Resolves the exact approved brief a script is built from. Cross-owner requests
-- are NOT_FOUND rather than FORBIDDEN so the ID space stays non-enumerable. The
-- brief must be completed, human-approved, immutable (its artifact and
-- provenance hashes are recomputed and compared, not trusted), integrity-valid
-- against its own upstream content-intelligence reference, and have passed the
-- Viewer Value gate.
-- ---------------------------------------------------------------------------
create or replace function channelwright.resolve_approved_video_brief_artifact(
  p_workflow_id uuid,
  p_run_id uuid
)
...
begin
  select * into v_run from channelwright.workflow_runs
    where id = p_run_id and workflow_id = p_workflow_id and workflow_type = 'CHANNEL_VIDEO_BRIEF';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_BRIEF run'; end if;
  select * into v_workflow from channelwright.workflows
    where id = p_workflow_id and owner_id = v_run.owner_id and workflow_type = 'CHANNEL_VIDEO_BRIEF';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_BRIEF workflow'; end if;
  if v_role <> 'service_role' and (auth.uid() is null or auth.uid() <> v_run.owner_id) then
    raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_BRIEF run';
  end if;

  if v_run.status <> 'COMPLETED' or v_run.output_payload is null then
    raise exception 'UPSTREAM_BRIEF_NOT_FINAL: the video brief must be completed and final';
  end if;
  ...
  if not found then raise exception 'UPSTREAM_BRIEF_NOT_APPROVED: exact video-brief run is not human approved'; end if;

  select output_payload into v_finalizer from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'finalize-video-brief' and status = 'COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'final-video-brief-qa' and status = 'COMPLETED';
  -- validate-approved-content is the brief's provenance step: it carries the exact
  -- upstream content-intelligence reference and the discovery evidence bundle
  -- every downstream claim must cite.
  select output_payload into v_provenance from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'validate-approved-content' and status = 'COMPLETED';

  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then
    raise exception 'UPSTREAM_BRIEF_INTEGRITY_MISMATCH: final output disagreement';
  end if;
  ...
  then raise exception 'UPSTREAM_BRIEF_QA_INVALID: final QA did not pass'; end if;
  ...
  then raise exception 'UPSTREAM_BRIEF_PROVENANCE_INVALID: discovery evidence is missing'; end if;
  -- The brief artifact must still agree with the content reference it was built
  -- on, which itself already carries the approved strategy and research chain.
  if v_run.output_payload->'upstreamContentIntelligence' is distinct from v_provenance->'reference' then
    raise exception 'UPSTREAM_BRIEF_INTEGRITY_MISMATCH: the brief no longer agrees with its upstream content reference';
  end if;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null
    ...
    raise exception 'UPSTREAM_BRIEF_INTEGRITY_MISMATCH: stored integrity data does not match';
  end if;

  begin v_parent := nullif(v_run.context_payload->>'previousRunId','')::uuid;
  exception when invalid_text_representation then raise exception 'UPSTREAM_BRIEF_LINEAGE_INVALID'; end;
  ...
  v_topic_id := v_run.output_payload->'selectedTopic'->>'topicId';
  v_pillar_id := v_run.output_payload->'selectedTopic'->>'pillarId';
  if v_topic_id is null or v_pillar_id is null then
    raise exception 'UPSTREAM_BRIEF_INTEGRITY_MISMATCH: the brief carries no selected topic identity';
  end if;

  -- Viewer Value eligibility. A brief that did not pass its own gate is not
  -- eligible to be scripted.
  v_gate := v_run.output_payload->'viewerValue'->>'gate';
  if v_gate is distinct from 'PASS' then
    raise exception 'UPSTREAM_BRIEF_VIEWER_VALUE_NOT_ELIGIBLE: the approved brief did not pass the Viewer Value gate';
  end if;

  return jsonb_build_object(
    'reference', jsonb_build_object(
      'briefWorkflowId', v_workflow.id,
      'briefRunId', v_run.id,
      ...
      'briefArtifactHash', v_artifact_hash,
      'briefProvenanceHash', v_provenance_hash,
      'parentRunId', v_parent,
      'rootRunId', v_root,
      'upstreamContentIntelligence', v_run.output_payload->'upstreamContentIntelligence'
    ),
    'briefResult', v_run.output_payload,
    'discoveryBundle', v_provenance->'discoveryBundle',
    'scope', jsonb_build_object(
      'briefTopicId', v_topic_id,
      ...
      -- brief's own viewer-value contract, so a script that quietly changes the
...
--
-- Reproduced from 202608150001 with the added CHANNEL_VIDEO_SCRIPT branch: the
-- earlier research, strategy, content, and video-brief branches, the idempotency
-- and advisory-lock behaviour, and the step-insertion loop are load-bearing for
-- every workflow and must not drift.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
  ...
  if p_workflow_type = 'CHANNEL_RESEARCH' then
    ...
    -- Only identifiers are accepted. The browser can never submit a brief object,
    -- a script, a provenance hash, or any QA state: those are resolved from
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the fifth paid workflow type.
--
-- Reproduced verbatim from 202608150001 apart from the added CHANNEL_VIDEO_SCRIPT
...
-- ---------------------------------------------------------------------------
-- Approval, revision lineage, and provenance hashing extend to the video script.
-- The video script's provenance step is validate-approved-brief: it carries the
-- exact upstream brief, the resolved topic, and the discovery bundle every
-- downstream claim must cite. A human-requested revision preserves the original
-- run, the pinned upstream reference, and root lineage.
--
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT') and v_old_run.output_payload is not null then
  ...
  if p_decision='REJECT' then
    ...
    insert into channelwright.workflow_events(owner_id,workflow_id,workflow_run_id,event_type,actor_type,actor_id,detail)
      values(v_owner,p_workflow_id,v_new_run_id,'WORKFLOW_REVISION_QUEUED','USER',v_owner::text,jsonb_build_object('previousRunId',v_old_run.id,'upstreamResearchRunId',v_old_run.input_payload->'approvedResearchReference'->>'researchRunId','upstreamStrategyRunId',v_old_run.input_payload->'approvedStrategyReference'->>'strategyRunId','upstreamContentRunId',v_old_run.input_payload->'approvedContentReference'->>'contentRunId','upstreamBriefRunId',v_old_run.input_payload->'approvedVideoBriefReference'->>'briefRunId','selectedTopicId',v_old_run.input_payload->>'selectedTopicId'));
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the fifth paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_SCRIPT joins CHANNEL_STRATEGY and CHANNEL_VIDEO_BRIEF as a
-- workflow that performs no external retrieval, so it is required to declare
-- exactly zero provider requests, quota units, and searches. Model-call and
-- token ceilings are unchanged from 202608130004.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT');
...
revoke all on function channelwright.resolve_approved_video_brief_artifact(uuid,uuid) from public,anon;
grant execute on function channelwright.resolve_approved_video_brief_artifact(uuid,uuid) to authenticated,service_role;
