-- CHANNEL_VIDEO_RELEASE vertical slice. Additive and forward-only: it extends the
...
--
-- CHANNEL_VIDEO_RELEASE is a DECISION and DURABLE-RECORD system, not a publishing
-- system. It consumes exactly one approved CHANNEL_VIDEO_PACKAGING artifact and
-- produces a human-approved, immutable release record: the selected title (an
-- exact packaging candidate), the selected thumbnail concept, a recommended
-- publish window, playlist/series placement, a distribution-surface plan, the
-- reconciled metadata, KPI/hypothesis bindings (identity and intent only), a
-- viewer-value/integrity assessment, and explicit human approval. It performs no
-- OAuth, upload, external scheduling, rendering, media/thumbnail/recut generation,
-- or performance ingestion.
--
-- Nothing here widens the database resource ceilings introduced in 202608130004.
-- Like packaging, this workflow performs no external evidence retrieval at all: it
-- reasons over the approved packaging and the discovery evidence that packaging's
-- upstream content-intelligence run already paid for, so its provider-request,
-- quota, and search ceilings are zero and only model calls consume budget.
--
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE'));

-- One active video-release run per owner per approved video-packaging run. A
-- packaging pins one video, so the packaging run alone is the key. BLOCKED is
-- excluded so a human-revision successor (predecessor moved to BLOCKED before the
-- successor is inserted) remains legal.
create unique index workflow_runs_active_video_release_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedVideoPackagingReference'->>'packagingRunId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_RELEASE' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');

-- ---------------------------------------------------------------------------
-- Approved video-packaging resolver
--
-- Resolves the exact approved packaging a release is built from. Cross-owner
-- requests are NOT_FOUND rather than FORBIDDEN so the ID space stays
-- non-enumerable. The packaging must be completed, human-approved, immutable (its
-- artifact and provenance hashes are recomputed and compared, not trusted),
-- integrity-valid against its own upstream video-script reference, and have passed
-- the Viewer Value gate. The resolved scope carries the authoritative selectable
-- title-candidate and thumbnail-concept identity sets, so the release's selection
-- is checked against database state, never the caller's word.
-- ---------------------------------------------------------------------------
...
--
-- Reproduced from 202608170001 with the added CHANNEL_VIDEO_RELEASE branch: the
-- earlier research, strategy, content, video-brief, video-script, and
-- video-packaging branches, the idempotency and advisory-lock behaviour, and the
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
  ...
  if p_workflow_type = 'CHANNEL_RESEARCH' then
    ...
    -- Only identifiers are accepted. The browser can never submit a packaging
    -- object, a release, a provenance hash, or any QA state: those are resolved
    -- from authoritative database state below.
    if jsonb_object_length(p_input) <> 2
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the seventh paid workflow type.
--
-- Reproduced verbatim from 202608170001 apart from the added
-- CHANNEL_VIDEO_RELEASE finalizer case: the service-role guard, idempotent-replay
-- path, lease and attempt checks, and the context_payload write that feeds
...
-- release. The video release's provenance step is validate-approved-packaging:
-- it carries the exact upstream packaging, the resolved scope, and the discovery
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE') and v_old_run.output_payload is not null then
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the seventh paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_RELEASE joins CHANNEL_STRATEGY, CHANNEL_VIDEO_BRIEF,
-- CHANNEL_VIDEO_SCRIPT, and CHANNEL_VIDEO_PACKAGING as a workflow that performs no
-- external retrieval, so it is required to declare exactly zero provider requests,
-- quota units, and searches. Model-call and token ceilings are unchanged from
-- 202608130004.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE');
