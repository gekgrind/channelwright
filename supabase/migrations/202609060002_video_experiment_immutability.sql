-- Extends approved-artifact immutability to CHANNEL_VIDEO_EXPERIMENT.
...
-- workflow_steps_protect_final_research) were last widened in 202609050002 to
-- include CHANNEL_VIDEO_DECISION. This migration adds CHANNEL_VIDEO_EXPERIMENT
-- to the same allow-list so an approved Experiment run's payload and finalized
...
begin
  if old.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION','CHANNEL_VIDEO_EXPERIMENT')
...
begin
  if new.output_payload is distinct from old.output_payload and exists(
    ...
    where r.id=old.workflow_run_id and r.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION','CHANNEL_VIDEO_EXPERIMENT') and a.status in ('APPROVED','REJECTED','REVISION_REQUESTED'))
