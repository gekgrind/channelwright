-- CHANNEL_VIDEO_PORTFOLIO vertical slice. Additive and forward-only: it extends
...
--
-- CHANNEL_VIDEO_PORTFOLIO is the capacity-allocation boundary after
-- CHANNEL_VIDEO_EXPERIMENT. Experiment deliberately reserved exactly two
-- downstream-facing contracts for this workflow -- `experimentReady` and
-- `portfolioEligible` -- and deterministically rejects, as
-- PORTFOLIO_RESPONSIBILITY_LEAKED, the work this workflow owns: allocating
-- finite operator capacity across experiments, prioritising which experiments run
-- next, and sequencing a slate.
--
-- It consumes ONE TO SIX exact approved, portfolio-eligible experiment artifacts
-- plus the operator's declared cycle capacity, and produces a single
-- human-approved, immutable ALLOCATION OF RECORD. It performs ZERO analytics
-- retrieval, ZERO experiment execution, and ZERO publishing / provider action. It
-- never redesigns an experiment, never revisits a Decision, and never rewrites
-- channel strategy.
--
-- The safety-critical structural property is in the resolver: it returns a
-- BOUNDED SERVER-EXTRACTED PROJECTION of each experiment (closed-vocabulary
-- scalars plus the 200-char title) instead of the experiment artifact. The
-- allocation model therefore never receives an experiment's mutable prose body
-- and is structurally incapable of restating or rewriting an approved design.
--
-- This is the first workflow whose upstream is a SET rather than a single
-- artifact. It is deliberately not a new mechanism: the same single-artifact
-- resolver is invoked once per candidate and every candidate is held to the
-- identical invariant list, including the round-3 root-lineage `workflow_id` pin
-- and the per-link supersession check Experiment established.
--
-- Nothing here widens the database resource ceilings introduced in 202608130004.
-- Like Diagnosis, Decision and Experiment, this workflow performs no external
-- evidence retrieval at all and makes zero automated revisions
...
-- One active portfolio allocation per owner per cycle. The key is the normalised
-- cycle label persisted by start_workflow, not a candidate set: two concurrent
-- allocations of the same cycle would each claim the same finite capacity, which
-- is exactly the double-spend this workflow exists to prevent. BLOCKED is
-- excluded so a human-revision successor (predecessor moved to BLOCKED before the
-- successor is inserted) remains legal.
create unique index workflow_runs_active_video_portfolio_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->>'portfolioCycleKey'))
  )
  where workflow_type = 'CHANNEL_VIDEO_PORTFOLIO' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');

-- ---------------------------------------------------------------------------
-- Exact approved-Experiment resolution for CHANNEL_VIDEO_PORTFOLIO. The RPC is
-- the sole authority for upstream identity, the portfolio-eligibility gate,
-- compact transitive lineage, and the bounded candidate projection.
--
-- Seventeen invariants, all pre-spend and identical in shape to the Decision
-- resolver Experiment established: existence + owner (cross-owner -> NOT_FOUND,
...
-- recommendation/score; the Experiment's own upstream-Decision provenance step
-- (validate-approved-decision) present and in agreement with the finalized
-- output; content.portfolioEligible = true (an ineligible Experiment can never
-- reach Portfolio -> UPSTREAM_EXPERIMENT_NOT_PORTFOLIO_ELIGIBLE); recomputed
-- artifact + provenance hashes match stored; parent/root lineage well-formed with
-- the root pinned to THIS EXACT Experiment workflow; and the full eleven-artifact
-- transitive chain (the ten the Experiment scope already carries, plus Experiment
-- itself) is re-hashed AND re-checked for supersession here rather than trusted.
--
-- The Experiment resolver's own semantic checks are not repeated -- they are
-- pinned by experimentProvenanceHash, which this re-hash re-derives.
--
-- The returned `candidate` object is the ONLY view of the experiment the
-- allocation model ever receives. Every field is a closed-vocabulary scalar
-- lifted from the immutable artifact, except `title`, which is bounded at the
-- 200 characters the experiment schema already enforced.
-- ---------------------------------------------------------------------------
create or replace function channelwright.resolve_approved_video_experiment_artifact(p_workflow_id uuid,p_run_id uuid)
...
declare
  ...
  v_scope_artifacts jsonb; v_full_artifacts jsonb; v_link jsonb; v_chain_run channelwright.workflow_runs%rowtype;
  v_out jsonb; v_src jsonb; v_exp jsonb;
  v_role text:=coalesce(auth.jwt()->>'role','');
begin
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_id=p_workflow_id and workflow_type='CHANNEL_VIDEO_EXPERIMENT';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_EXPERIMENT run'; end if;
  select * into v_workflow from channelwright.workflows where id=p_workflow_id and owner_id=v_run.owner_id and workflow_type='CHANNEL_VIDEO_EXPERIMENT';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_EXPERIMENT workflow'; end if;
  if v_role<>'service_role' and (auth.uid() is null or auth.uid()<>v_run.owner_id) then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_EXPERIMENT run'; end if;
  if v_run.status<>'COMPLETED' or v_run.output_payload is null then raise exception 'UPSTREAM_EXPERIMENT_NOT_FINAL: Experiment must be completed and final'; end if;
  if v_workflow.current_run_id is distinct from v_run.id or exists(select 1 from channelwright.workflow_runs r where r.workflow_id=v_run.workflow_id and r.owner_id=v_run.owner_id and r.context_payload->>'previousRunId'=v_run.id::text)
    then raise exception 'UPSTREAM_EXPERIMENT_SUPERSEDED: Experiment is not the latest immutable successor'; end if;
  ...
  if not found then raise exception 'UPSTREAM_EXPERIMENT_NOT_APPROVED: exact Experiment run is not human approved'; end if;
  select output_payload into v_finalizer from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='finalize-video-experiment' and status='COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='final-video-experiment-qa' and status='COMPLETED';
  select output_payload into v_provenance from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='validate-approved-decision' and status='COMPLETED';
  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then raise exception 'UPSTREAM_EXPERIMENT_INTEGRITY_MISMATCH: finalizer output disagreement'; end if;
  if v_final_qa is null or coalesce((v_final_qa->'qa'->>'passed')::boolean,false) is not true or v_final_qa->'qa'->>'recommendation' not in ('accept','human_review_required') or coalesce((v_final_qa->'qa'->>'score')::integer,-1) not between 0 and 100
    then raise exception 'UPSTREAM_EXPERIMENT_QA_INVALID: final QA did not pass'; end if;
  if v_provenance is null or v_provenance->'reference' is null then raise exception 'UPSTREAM_EXPERIMENT_PROVENANCE_INVALID: authoritative Decision provenance is missing'; end if;
  if v_run.output_payload->'approvedVideoDecisionReference' is distinct from v_provenance->'reference' then raise exception 'UPSTREAM_EXPERIMENT_INTEGRITY_MISMATCH: nested Decision reference drift'; end if;
  if v_run.output_payload#>'{content,portfolioEligible}' is distinct from 'true'::jsonb then raise exception 'UPSTREAM_EXPERIMENT_NOT_PORTFOLIO_ELIGIBLE: the approved Experiment is not eligible for portfolio allocation'; end if;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null or v_run.artifact_hash<>v_artifact_hash or v_run.provenance_hash<>v_provenance_hash then raise exception 'UPSTREAM_EXPERIMENT_INTEGRITY_MISMATCH: canonical hash mismatch'; end if;
  begin v_parent:=nullif(v_run.context_payload->>'previousRunId','')::uuid; exception when invalid_text_representation then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: malformed parent'; end;
  ...
  if v_parent is not null and not exists(select 1 from channelwright.workflow_runs where id=v_parent and workflow_id=v_run.workflow_id and owner_id=v_run.owner_id and workflow_type=v_run.workflow_type) then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: parent/root drift'; end if;
  -- Authoritative parent/root lineage, carrying forward the round-3 hardening
  -- Experiment established: when the Experiment run carries an accounting budget,
  -- its recorded parent must agree with context_payload's previousRunId, and the
  -- resolved root must itself be a real owner-scoped run of THIS EXACT Experiment
  -- workflow -- not merely some same-owner CHANNEL_VIDEO_EXPERIMENT run in a
  -- different workflow.
  if coalesce(v_budget_found,false) and v_budget_parent is distinct from v_parent then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: budget parent disagrees with lineage parent'; end if;
  if v_root is not null and not exists(select 1 from channelwright.workflow_runs where id=v_root and owner_id=v_run.owner_id and workflow_type=v_run.workflow_type and workflow_id=v_run.workflow_id) then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: resolved root is not a real run of this workflow'; end if;

  v_scope_artifacts:=v_run.output_payload->'experimentScope'->'artifacts';
  if v_scope_artifacts is null or jsonb_array_length(v_scope_artifacts)<>10 then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: experiment scope must carry exactly ten upstream artifacts'; end if;
  v_full_artifacts:=v_scope_artifacts || jsonb_build_array(jsonb_build_object('workflowType','CHANNEL_VIDEO_EXPERIMENT','runId',v_run.id,'artifactHash',v_artifact_hash,'schemaVersion',(v_run.output_payload->>'schemaVersion')::integer));
  for v_link in select value from jsonb_array_elements(v_full_artifacts) loop
    ...
    if not found or v_chain_run.status<>'COMPLETED' or v_chain_run.output_payload is null or encode(digest(v_chain_run.output_payload::text,'sha256'),'hex')<>v_link->>'artifactHash' or (v_chain_run.output_payload->>'schemaVersion')::integer<>(v_link->>'schemaVersion')::integer then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: transitive artifact mismatch'; end if;
    -- Every transitive link must also be the current, non-superseded run of its
    -- own workflow -- the identical invariant the direct Experiment run is held to
    -- above. A structurally superseded upstream must not be laundered into an
    -- approvable Portfolio lineage just because its frozen hash still matches the
    ...
    if not exists(select 1 from channelwright.workflows w where w.id=v_chain_run.workflow_id and w.owner_id=v_run.owner_id and w.current_run_id=v_chain_run.id)
      ...
      then raise exception 'UPSTREAM_EXPERIMENT_LINEAGE_INVALID: transitive artifact superseded'; end if;
  end loop;

  v_out:=v_run.output_payload; v_src:=v_out->'source'; v_exp:=v_out#>'{content,experiment}';
  if v_src is null or v_exp is null then raise exception 'UPSTREAM_EXPERIMENT_INTEGRITY_MISMATCH: experiment artifact is missing its canonical projection surfaces'; end if;

  return jsonb_build_object('reference',jsonb_build_object(
    'experimentWorkflowId',v_workflow.id,'experimentRunId',v_run.id,'workflowDefinitionVersion',v_run.definition_version,'outputSchemaVersion',(v_out->>'schemaVersion')::integer,
    ...
    'finalQaState',v_final_qa->'qa'->>'recommendation','finalQaScore',(v_final_qa->'qa'->>'score')::integer,'experimentArtifactHash',v_artifact_hash,'experimentProvenanceHash',v_provenance_hash,
    'parentRunId',v_parent,'rootRunId',v_root,
    'experimentType',v_exp->>'experimentType','portfolioEligible',true,
    -- A FLAT upstream summary, not the nested Decision reference. A portfolio
    -- carries up to six candidates, and six copies of the ten-deep sibling
    -- reference chain exceed the 64 KiB step-output ceiling on their own. The
    -- decision identity and both hashes are preserved here; run-by-run lineage
    -- lives on the candidate projection and every chain hash in `artifacts`.
    'upstreamVideoDecision',jsonb_build_object(
      'decisionWorkflowId',v_out#>>'{approvedVideoDecisionReference,decisionWorkflowId}',
      'decisionRunId',v_out#>>'{approvedVideoDecisionReference,decisionRunId}',
      'decisionArtifactHash',v_out#>>'{approvedVideoDecisionReference,decisionArtifactHash}',
      'decisionProvenanceHash',v_out#>>'{approvedVideoDecisionReference,decisionProvenanceHash}',
      'decisionType',v_out#>>'{approvedVideoDecisionReference,decisionType}',
      'experimentEligible',true)),
    'candidate',jsonb_build_object(
      'candidateId','cand:'||v_run.id::text,
      'experimentWorkflowId',v_workflow.id,
      'experimentRunId',v_run.id,
      'experimentId',v_exp->>'id',
      'title',v_exp->>'title',
      'experimentType',v_exp->>'experimentType',
      'disposition',v_exp->>'disposition',
      'measurementOnly',v_exp->'measurementOnly',
      'controlKind',v_exp#>>'{controlCondition,kind}',
      'category',v_exp->>'category',
      'unitOfAssignment',v_exp->>'unitOfAssignment',
      'treatmentMechanism',v_exp#>>'{semanticIntent,treatmentMechanism}',
      'primaryMetric',v_exp#>>'{primaryMetric,metric}',
      'primaryMetricDirection',v_exp#>>'{primaryMetric,direction}',
      'evidenceStrength',v_exp->>'evidenceStrength',
      'confidenceInDesign',v_exp->>'confidenceInDesign',
      'requiresHumanJudgment',v_exp->'requiresHumanJudgment',
      'experimentReady',v_out#>'{content,experimentReady}',
      'viewerValueState',v_out#>>'{content,viewerValueSafeguards,inheritedState}',
      'promiseIntegrityRisk',v_out#>>'{content,viewerValueSafeguards,promiseIntegrityRisk}',
      'escalationRequired',v_out#>'{content,viewerValueSafeguards,escalationRequired}',
      'evidenceCanChangeShippingDecision',v_exp#>'{semanticIntent,evidenceCanChangeShippingDecision}',
      'decisionId',v_exp#>>'{decisionLinkage,decisionId}',
      'decisionType',v_exp#>>'{decisionLinkage,decisionType}',
      'viewerValueContractHash',v_out#>>'{viewerValueProvenance,contractHash}',
      'lineage',jsonb_build_object(
        'decisionRunId',v_src->>'decisionRunId',
        'diagnosisRunId',v_src->>'diagnosisRunId',
        'performanceRunId',v_src->>'performanceRunId',
        'releaseRunId',v_src->>'releaseRunId',
        'topicId',v_src->>'topicId',
        'pillarId',v_src->>'pillarId',
        'finalTitle',v_src->>'finalTitle')),
    'artifacts',v_full_artifacts);
end $$;

-- ---------------------------------------------------------------------------
-- Canonical graph registration, input validation, and concurrency guards.
--
-- Reproduced from 202609060001 with the added CHANNEL_VIDEO_PORTFOLIO branch:
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the eleventh paid workflow type.
--
-- Reproduced verbatim from 202609060001 apart from the added
-- CHANNEL_VIDEO_PORTFOLIO finalizer case: the service-role guard, idempotent-
...
-- ---------------------------------------------------------------------------
-- Approval, revision lineage, and provenance hashing extend to the allocation
-- record. Its provenance step is validate-approved-experiments: it carries the
-- exact upstream Experiment reference set and the portfolio-scope lineage every
-- downstream citation must resolve against. A human-requested revision preserves
-- the original run, the pinned upstream references, and root lineage.
--
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the eleventh paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_PORTFOLIO joins CHANNEL_VIDEO_DIAGNOSIS, CHANNEL_VIDEO_DECISION
-- and CHANNEL_VIDEO_EXPERIMENT as a workflow that performs no external retrieval
...
begin
  ...
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION','CHANNEL_VIDEO_EXPERIMENT','CHANNEL_VIDEO_PORTFOLIO');
  v_tight_ceiling := v_run.workflow_type in ('CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION','CHANNEL_VIDEO_EXPERIMENT','CHANNEL_VIDEO_PORTFOLIO');
