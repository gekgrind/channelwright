-- Extends approved-artifact immutability to CHANNEL_VIDEO_INTELLIGENCE.
...
-- workflow_steps_protect_final_research) were last widened in 202609070002 to
-- include CHANNEL_VIDEO_PORTFOLIO. This migration adds
-- CHANNEL_VIDEO_INTELLIGENCE to the same allow-list so an approved channel
-- learning record's payload and finalized step output cannot be rewritten by a
-- privileged/service-role write either.
--
-- This matters more here than anywhere else on the spine: the learning record is
-- the channel's durable memory and the anchor for a strategy review signal. A
-- record that could be edited after approval would let the channel quietly
-- rewrite what it claims to have learned, which is exactly the decision-history
-- integrity the Product Doctrine requires be preserved.
--
