-- CHANNEL_VIDEO_PACKAGING vertical slice. Additive and forward-only: it extends
...
-- the approved video script and the discovery evidence that script's upstream
-- content-intelligence run already paid for, so its provider-request, quota, and
-- search ceilings are zero and only model calls consume budget.
--
...
-- matches the state 202608150002 established for the sibling resolvers and
-- avoids a follow-up repair. `public` stays excluded throughout.
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING'));

-- One active video-packaging run per owner per approved video-script run. A
-- script pins one video, so the script run alone is the key. BLOCKED is excluded
-- so a human-revision successor (predecessor moved to BLOCKED before the successor
-- is inserted) remains legal.
create unique index workflow_runs_active_video_packaging_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedVideoScriptReference'->>'scriptRunId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_PACKAGING' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');

-- ---------------------------------------------------------------------------
-- Approved video-script resolver
--
-- Resolves the exact approved script a packaging is built from. Cross-owner
-- requests are NOT_FOUND rather than FORBIDDEN so the ID space stays
-- non-enumerable. The script must be completed, human-approved, immutable (its
-- artifact and provenance hashes are recomputed and compared, not trusted),
-- integrity-valid against its own upstream video-brief reference, and have passed
-- the Viewer Value gate.
-- ---------------------------------------------------------------------------
create or replace function channelwright.resolve_approved_video_script_artifact(
  p_workflow_id uuid,
  p_run_id uuid
)
...
begin
  select * into v_run from channelwright.workflow_runs
    where id = p_run_id and workflow_id = p_workflow_id and workflow_type = 'CHANNEL_VIDEO_SCRIPT';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_SCRIPT run'; end if;
  select * into v_workflow from channelwright.workflows
    where id = p_workflow_id and owner_id = v_run.owner_id and workflow_type = 'CHANNEL_VIDEO_SCRIPT';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_SCRIPT workflow'; end if;
  if v_role <> 'service_role' and (auth.uid() is null or auth.uid() <> v_run.owner_id) then
    raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_SCRIPT run';
  end if;

  if v_run.status <> 'COMPLETED' or v_run.output_payload is null then
    raise exception 'UPSTREAM_SCRIPT_NOT_FINAL: the video script must be completed and final';
  end if;
  ...
  if not found then raise exception 'UPSTREAM_SCRIPT_NOT_APPROVED: exact video-script run is not human approved'; end if;

  select output_payload into v_finalizer from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'finalize-video-script' and status = 'COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'final-video-script-qa' and status = 'COMPLETED';
  -- validate-approved-brief is the script's provenance step: it carries the exact
  -- upstream video-brief reference and the discovery evidence bundle every
  -- downstream claim must cite.
  select output_payload into v_provenance from channelwright.workflow_steps
    where workflow_run_id = v_run.id and step_key = 'validate-approved-brief' and status = 'COMPLETED';

  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then
    raise exception 'UPSTREAM_SCRIPT_INTEGRITY_MISMATCH: final output disagreement';
  end if;
  ...
  then raise exception 'UPSTREAM_SCRIPT_QA_INVALID: final QA did not pass'; end if;
  ...
  then raise exception 'UPSTREAM_SCRIPT_PROVENANCE_INVALID: discovery evidence is missing'; end if;
  -- The script artifact must still agree with the video-brief reference it was
  -- built on, which itself already carries the approved content/strategy/research
  -- chain.
  if v_run.output_payload->'upstreamVideoBrief' is distinct from v_provenance->'reference' then
    raise exception 'UPSTREAM_SCRIPT_INTEGRITY_MISMATCH: the script no longer agrees with its upstream brief reference';
  end if;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null
    ...
    raise exception 'UPSTREAM_SCRIPT_INTEGRITY_MISMATCH: stored integrity data does not match';
  end if;

  begin v_parent := nullif(v_run.context_payload->>'previousRunId','')::uuid;
  exception when invalid_text_representation then raise exception 'UPSTREAM_SCRIPT_LINEAGE_INVALID'; end;
  ...
  v_topic_id := v_run.output_payload->'scriptScope'->>'briefTopicId';
  v_pillar_id := v_run.output_payload->'scriptScope'->>'pillarId';
  v_duration := (v_run.output_payload->'timing'->>'totalDurationSeconds')::integer;
  if v_topic_id is null or v_pillar_id is null or v_duration is null then
    raise exception 'UPSTREAM_SCRIPT_INTEGRITY_MISMATCH: the script carries no scope or timing identity';
  end if;

  -- The authoritative script timeline packaging chapters must derive from.
  select jsonb_agg(jsonb_build_object(
    'sectionId', s->>'sectionId',
    'title', s->>'title',
    'startSeconds', (s->>'startSeconds')::integer,
    'durationSeconds', (s->>'durationSeconds')::integer
  ) order by (s->>'startSeconds')::integer)
  into v_timing
  from jsonb_array_elements(v_run.output_payload->'sections') s;
  if v_timing is null then
    raise exception 'UPSTREAM_SCRIPT_INTEGRITY_MISMATCH: the script carries no section timing';
  end if;

  -- Viewer Value eligibility. A script that did not pass its own gate is not
  -- eligible to be packaged.
  v_gate := v_run.output_payload->'viewerValue'->>'gate';
  if v_gate is distinct from 'PASS' then
    raise exception 'UPSTREAM_SCRIPT_VIEWER_VALUE_NOT_ELIGIBLE: the approved script did not pass the Viewer Value gate';
  end if;

  return jsonb_build_object(
    'reference', jsonb_build_object(
      'scriptWorkflowId', v_workflow.id,
      'scriptRunId', v_run.id,
      ...
      'scriptArtifactHash', v_artifact_hash,
      'scriptProvenanceHash', v_provenance_hash,
      'parentRunId', v_parent,
      'rootRunId', v_root,
      'upstreamVideoBrief', v_run.output_payload->'upstreamVideoBrief'
    ),
    'scriptResult', v_run.output_payload,
    'discoveryBundle', v_provenance->'discoveryBundle',
    'scope', jsonb_build_object(
      'scriptTopicId', v_topic_id,
      'pillarId', v_pillar_id,
      'scriptDurationSeconds', v_duration,
      'scriptTiming', v_timing,
      ...
      -- script's own viewer-value contract, so packaging that quietly changes the
      -- promise no longer matches its source.
...
--
-- Reproduced from 202608160001 with the added CHANNEL_VIDEO_PACKAGING branch:
-- the earlier research, strategy, content, video-brief, and video-script
-- branches, the idempotency and advisory-lock behaviour, and the step-insertion
-- loop are load-bearing for every workflow and must not drift.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
  ...
  if p_workflow_type = 'CHANNEL_RESEARCH' then
    ...
    -- Only identifiers are accepted. The browser can never submit a script object,
    -- a packaging, a provenance hash, or any QA state: those are resolved from
    -- authoritative database state below.
    if jsonb_object_length(p_input) <> 2
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the sixth paid workflow type.
--
-- Reproduced verbatim from 202608160001 apart from the added
-- CHANNEL_VIDEO_PACKAGING finalizer case: the service-role guard, idempotent-
-- replay path, lease and attempt checks, and the context_payload write that feeds
-- priorOutputs are all load-bearing for every workflow and must not drift.
-- ---------------------------------------------------------------------------
...
-- packaging. The video packaging's provenance step is validate-approved-script:
-- it carries the exact upstream script, the resolved scope, and the discovery
-- bundle every downstream claim must cite. A human-requested revision preserves
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING') and v_old_run.output_payload is not null then
  ...
  if p_decision='REJECT' then
    ...
    insert into channelwright.workflow_events(owner_id,workflow_id,workflow_run_id,event_type,actor_type,actor_id,detail)
      values(v_owner,p_workflow_id,v_new_run_id,'WORKFLOW_REVISION_QUEUED','USER',v_owner::text,jsonb_build_object('previousRunId',v_old_run.id,'upstreamResearchRunId',v_old_run.input_payload->'approvedResearchReference'->>'researchRunId','upstreamStrategyRunId',v_old_run.input_payload->'approvedStrategyReference'->>'strategyRunId','upstreamContentRunId',v_old_run.input_payload->'approvedContentReference'->>'contentRunId','upstreamBriefRunId',v_old_run.input_payload->'approvedVideoBriefReference'->>'briefRunId','upstreamScriptRunId',v_old_run.input_payload->'approvedVideoScriptReference'->>'scriptRunId','selectedTopicId',v_old_run.input_payload->>'selectedTopicId'));
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the sixth paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_PACKAGING joins CHANNEL_STRATEGY, CHANNEL_VIDEO_BRIEF, and
-- CHANNEL_VIDEO_SCRIPT as a workflow that performs no external retrieval, so it
-- is required to declare exactly zero provider requests, quota units, and
-- searches. Model-call and token ceilings are unchanged from 202608130004.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING');
...
revoke all on function channelwright.resolve_approved_video_script_artifact(uuid,uuid) from public,anon;
grant execute on function channelwright.resolve_approved_video_script_artifact(uuid,uuid) to authenticated,service_role;
