-- Extends approved-artifact immutability to CHANNEL_VIDEO_PERFORMANCE.
...
-- workflow_steps_protect_final_research) were last widened in 202608180002 to
...
-- 'CHANNEL_VIDEO_RELEASE'). The video-performance migration hashes approved state
-- but did not widen that list, so a privileged/service-role write could rewrite
-- an approved VIDEO_PERFORMANCE run payload or a finalized step output and
-- re-stamp its hashes.
--
...
begin
  if old.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE')
...
begin
  if new.output_payload is distinct from old.output_payload and exists(
    ...
    where r.id=old.workflow_run_id and r.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE') and a.status in ('APPROVED','REJECTED','REVISION_REQUESTED'))
