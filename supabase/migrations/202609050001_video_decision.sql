-- CHANNEL_VIDEO_DECISION vertical slice. Additive and forward-only: it extends
...
--
-- CHANNEL_VIDEO_DECISION is the human decision-authority boundary after one
-- exact approved CHANNEL_VIDEO_DIAGNOSIS artifact. It converts evidence-
-- supported conditions and uncertainty into a single chosen direction from a
-- bounded taxonomy (PRESERVE_CURRENT_APPROACH, GATHER_EVIDENCE, INVESTIGATE,
-- PRIORITIZE_CHANGE, DEFER, ESCALATE_TO_HUMAN_JUDGMENT), with cited findings,
-- ranked alternatives, and a confidence the evidence actually entitles it to.
-- It performs ZERO analytics retrieval, ZERO experiment design, and ZERO
-- execution / publishing / content mutation. It never revises Diagnosis: the
-- only Experiment-facing contract it defines is the derived boolean
-- `experimentEligible`.
--
-- Nothing here widens the database resource ceilings introduced in 202608130004.
-- Like Diagnosis, this workflow performs no external evidence retrieval at all:
-- it reasons only over the approved Diagnosis artifact. Its provider-request,
-- quota, and search ceilings are zero, and it makes zero automated revisions
-- (maxAutomatedRevisions = 0, matching CHANNEL_VIDEO_DIAGNOSIS).
--
...
alter table channelwright.workflows add constraint workflows_workflow_type_check
  check (workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION'));
...
alter table channelwright.workflow_runs add constraint workflow_runs_workflow_type_check
  check (workflow_type in ('CHANNEL','VIDEO','CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION'));

-- One active video-decision run per owner per approved Diagnosis run. BLOCKED
...
-- decision on the same Diagnosis is a fresh sequential run started after the
-- first decision run for that Diagnosis completes.
create unique index workflow_runs_active_video_decision_uniq
  on channelwright.workflow_runs (
    owner_id,
    ((input_payload->'approvedVideoDiagnosisReference'->>'diagnosisRunId'))
  )
  where workflow_type = 'CHANNEL_VIDEO_DECISION' and status in ('QUEUED','RUNNING','WAITING_FOR_APPROVAL','PAUSED');
...
-- with a legal recommendation/score; the Diagnosis's own upstream-Performance
-- provenance step (validate-approved-performance) present and in agreement
...
-- duplication.
-- ---------------------------------------------------------------------------
create or replace function channelwright.resolve_approved_video_diagnosis_artifact(p_workflow_id uuid,p_run_id uuid)
...
begin
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_id=p_workflow_id and workflow_type='CHANNEL_VIDEO_DIAGNOSIS';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_DIAGNOSIS run'; end if;
  select * into v_workflow from channelwright.workflows where id=p_workflow_id and owner_id=v_run.owner_id and workflow_type='CHANNEL_VIDEO_DIAGNOSIS';
  if not found then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_DIAGNOSIS workflow'; end if;
  if v_role<>'service_role' and (auth.uid() is null or auth.uid()<>v_run.owner_id) then raise exception 'NOT_FOUND: exact CHANNEL_VIDEO_DIAGNOSIS run'; end if;
  if v_run.status<>'COMPLETED' or v_run.output_payload is null then raise exception 'UPSTREAM_DIAGNOSIS_NOT_FINAL: Diagnosis must be completed and final'; end if;
  if v_workflow.current_run_id is distinct from v_run.id or exists(select 1 from channelwright.workflow_runs r where r.workflow_id=v_run.workflow_id and r.owner_id=v_run.owner_id and r.context_payload->>'previousRunId'=v_run.id::text)
    then raise exception 'UPSTREAM_DIAGNOSIS_SUPERSEDED: Diagnosis is not the latest immutable successor'; end if;
  ...
  if not found then raise exception 'UPSTREAM_DIAGNOSIS_NOT_APPROVED: exact Diagnosis run is not human approved'; end if;
  select output_payload into v_finalizer from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='finalize-video-diagnosis' and status='COMPLETED';
  select output_payload into v_final_qa from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='final-video-diagnosis-qa' and status='COMPLETED';
  select output_payload into v_provenance from channelwright.workflow_steps where workflow_run_id=v_run.id and step_key='validate-approved-performance' and status='COMPLETED';
  if v_finalizer is null or v_finalizer is distinct from v_run.output_payload then raise exception 'UPSTREAM_DIAGNOSIS_INTEGRITY_MISMATCH: finalizer output disagreement'; end if;
  if v_final_qa is null or coalesce((v_final_qa->'qa'->>'passed')::boolean,false) is not true or v_final_qa->'qa'->>'recommendation' not in ('accept','human_review_required') or coalesce((v_final_qa->'qa'->>'score')::integer,-1) not between 0 and 100
    then raise exception 'UPSTREAM_DIAGNOSIS_QA_INVALID: final QA did not pass'; end if;
  if v_provenance is null or v_provenance->'reference' is null then raise exception 'UPSTREAM_DIAGNOSIS_PROVENANCE_INVALID: authoritative Performance provenance is missing'; end if;
  if v_run.output_payload->'upstreamVideoPerformance' is distinct from v_provenance->'reference' then raise exception 'UPSTREAM_DIAGNOSIS_INTEGRITY_MISMATCH: nested Performance reference drift'; end if;
  if v_run.output_payload#>'{analysis,summary,decisionDeferred}' is distinct from 'true'::jsonb then raise exception 'UPSTREAM_DIAGNOSIS_INTEGRITY_MISMATCH: Diagnosis did not defer its decision'; end if;
  ...
  if v_run.artifact_hash is null or v_run.provenance_hash is null or v_run.artifact_hash<>v_artifact_hash or v_run.provenance_hash<>v_provenance_hash then raise exception 'UPSTREAM_DIAGNOSIS_INTEGRITY_MISMATCH: canonical hash mismatch'; end if;
  begin v_parent:=nullif(v_run.context_payload->>'previousRunId','')::uuid; exception when invalid_text_representation then raise exception 'UPSTREAM_DIAGNOSIS_LINEAGE_INVALID: malformed parent'; end;
  ...
  if v_parent is not null and not exists(select 1 from channelwright.workflow_runs where id=v_parent and workflow_id=v_run.workflow_id and owner_id=v_run.owner_id and workflow_type=v_run.workflow_type) then raise exception 'UPSTREAM_DIAGNOSIS_LINEAGE_INVALID: parent/root drift'; end if;

  v_scope_artifacts:=v_run.output_payload->'diagnosisScope'->'artifacts';
  if v_scope_artifacts is null or jsonb_array_length(v_scope_artifacts)<>8 then raise exception 'UPSTREAM_DIAGNOSIS_LINEAGE_INVALID: diagnosis scope must carry exactly eight upstream artifacts'; end if;
  v_full_artifacts:=v_scope_artifacts || jsonb_build_array(jsonb_build_object('workflowType','CHANNEL_VIDEO_DIAGNOSIS','runId',v_run.id,'artifactHash',v_artifact_hash,'schemaVersion',(v_run.output_payload->>'schemaVersion')::integer));
  ...
  v_facts:=coalesce(v_run.output_payload->'diagnosisScope'->'facts','[]'::jsonb) || jsonb_build_array(
    jsonb_build_object('key','fact:diagnosis-outcome','value',v_run.output_payload#>'{analysis,summary,outcome}','sourceRef','diagnosis:/analysis/summary/outcome'),
    jsonb_build_object('key','fact:diagnosis-overall-confidence','value',v_run.output_payload#>'{analysis,summary,overallConfidence}','sourceRef','diagnosis:/analysis/summary/overallConfidence'));
  if exists(select 1 from jsonb_array_elements(v_facts) x group by x->>'key' having count(*)>1) then raise exception 'UPSTREAM_DIAGNOSIS_LINEAGE_INVALID: duplicate projected fact key'; end if;

  return jsonb_build_object('reference',jsonb_build_object(
    'diagnosisWorkflowId',v_workflow.id,'diagnosisRunId',v_run.id,'workflowDefinitionVersion',v_run.definition_version,'outputSchemaVersion',(v_run.output_payload->>'schemaVersion')::integer,
    ...
    'finalQaState',v_final_qa->'qa'->>'recommendation','finalQaScore',(v_final_qa->'qa'->>'score')::integer,'diagnosisArtifactHash',v_artifact_hash,'diagnosisProvenanceHash',v_provenance_hash,
    'parentRunId',v_parent,'rootRunId',v_root,'upstreamVideoPerformance',v_run.output_payload->'upstreamVideoPerformance'),
    'diagnosisResult',v_run.output_payload,
    'decisionScope',jsonb_build_object('artifacts',v_full_artifacts,'entries',coalesce(v_run.output_payload->'diagnosisScope'->'entries','[]'::jsonb),'facts',v_facts));
end $$;

-- ---------------------------------------------------------------------------
-- Canonical graph registration, input validation, and concurrency guards.
--
-- Reproduced from 202609040001 with the added CHANNEL_VIDEO_DECISION branch:
...
begin
  ...
  if not (p_definition_version = 1 and p_workflow_type in ('CHANNEL_CONCEPT_VALIDATION','CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION')) then raise exception 'WORKFLOW_TYPE_INVALID: unsupported workflow definition'; end if;
...
-- ---------------------------------------------------------------------------
-- Finalizer promotion for the ninth paid workflow type.
--
-- Reproduced verbatim from 202609040001 apart from the added
-- CHANNEL_VIDEO_DECISION finalizer case: the service-role guard, idempotent-
-- replay path, lease and attempt checks, and the context_payload write that
-- feeds priorOutputs are all load-bearing for every workflow and must not
-- drift.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Approval, revision lineage, and provenance hashing extend to the decision
-- record. The decision record's provenance step is validate-approved-
-- diagnosis: it carries the exact upstream Diagnosis reference and the
-- decision-scope lineage every downstream citation must resolve against. A
-- human-requested revision preserves the original run, the pinned upstream
-- reference, and root lineage.
--
...
begin
  ...
  if v_old_run.workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION') and v_old_run.output_payload is not null then
...
-- ---------------------------------------------------------------------------
-- Durable accounting accepts the ninth paid workflow type. Ceilings unchanged.
--
-- CHANNEL_VIDEO_DECISION joins CHANNEL_VIDEO_DIAGNOSIS as a workflow that
-- performs no external retrieval and no automated revision, so it shares
-- Diagnosis's tighter 2/2/0/0 synthesis/QA/revision/automated-revision
-- ceilings rather than the wider ceilings the revision-capable verticals use.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select * into v_run from channelwright.workflow_runs where id=p_run_id and workflow_type in ('CHANNEL_RESEARCH','CHANNEL_STRATEGY','CHANNEL_CONTENT_INTELLIGENCE','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION');
  if not found then raise exception 'NOT_FOUND: paid workflow run'; end if;
  v_no_retrieval := v_run.workflow_type in ('CHANNEL_STRATEGY','CHANNEL_VIDEO_BRIEF','CHANNEL_VIDEO_SCRIPT','CHANNEL_VIDEO_PACKAGING','CHANNEL_VIDEO_RELEASE','CHANNEL_VIDEO_PERFORMANCE','CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION');
  v_tight_ceiling := v_run.workflow_type in ('CHANNEL_VIDEO_DIAGNOSIS','CHANNEL_VIDEO_DECISION');
