/**
 * Disposable PostgreSQL runtime gate for the CHANNEL_VIDEO_PERFORMANCE migration
 * chain.
 *
...
import { approvedVideoReleaseArtifactFixture } from "../src/server/workflows/video-performance-fixtures.test-helper";
...
async function seedApprovedRun(db: pg.Client, ownerId: string, type: string, output: unknown, provStep: string, provOutput: unknown, opts: { approve?: boolean; corruptHash?: boolean; finalizerStep: string; finalQaStep: string } = { finalizerStep: "", finalQaStep: "" }) {
  ...
  if (finalQaStep) steps.push([finalQaStep, { qa: { passed: true, score: 88, findings: [], recommendation: "accept", deterministicChecksPassed: 30, deterministicChecksFailed: 0, modelUsage: { model: "g", inputTokens: 1, outputTokens: 1, totalTokens: 2 } } }]);
...
async function runChecks(db: pg.Client) {
  const release = approvedVideoReleaseArtifactFixture.releaseResult;
  const discovery = approvedVideoReleaseArtifactFixture.discoveryBundle;
  const packagingRef = release.upstreamVideoPackaging;
  // The release's provenance step output (validate-approved-packaging) carries
  // the exact upstream packaging reference and the discovery evidence bundle.
  const provenance = { reference: packagingRef, packagingResult: {}, discoveryBundle: discovery, scope: {} };
  ...
  const fns = await db.query<{ count: string }>(
    "select count(*)::text as count from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='channelwright' and p.proname in ('resolve_approved_research_artifact','resolve_approved_strategy_artifact','resolve_approved_content_artifact','resolve_approved_video_brief_artifact','resolve_approved_video_script_artifact','resolve_approved_video_packaging_artifact','resolve_approved_video_release_artifact')");
  record("all seven approved-artifact resolvers compiled", fns.rows[0].count === "7", `count=${fns.rows[0].count}`);

  // --- canonical parity --------------------------------------------------
  const contract = release.viewerValue.contract;
  ...
  // --- resolver valid ----------------------------------------------------
  const seeded = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_RELEASE", release, "validate-approved-packaging", provenance, { finalizerStep: "finalize-video-release", finalQaStep: "final-video-release-qa" });
  const resolved = await asOwner(db, ownerA, () => db.query<{ result: Record<string, unknown> }>("select channelwright.resolve_approved_video_release_artifact($1,$2) as result", [seeded.workflowId, seeded.runId]));
  ...
  const strategyRunId = release.upstreamVideoPackaging.upstreamVideoScript.upstreamVideoBrief.upstreamContentIntelligence.upstreamStrategy.strategyRunId;
  record("resolver returns scope for the approved release owner", typeof scope.releaseTopicId === "string");
  record("resolver carries the release promise and final title", scope.releasePromise === release.source.releasePromise && scope.finalTitle === release.reconciledMetadata.finalTitle);
  record("resolver anchors scope to the transitive strategy identity", scope.strategyRunId === strategyRunId);
  record("resolver projects the release's bound KPI/hypothesis set", Array.isArray(scope.kpiBindings) && (scope.kpiBindings as unknown[]).length === release.kpiHypothesisBindings.length);
  record("inherited Viewer Value contract hash matches app canonical hash", prov.contractHash === sha256(canonicalJson(contract)));
  record("inherited provenance records RELEASE origin and PASS gate", prov.originStage === "RELEASE" && prov.gate === "PASS");

  // --- resolver invalid --------------------------------------------------
  record("cross-owner resolution is NOT_FOUND", (await expectFailure(db, ownerB, "select channelwright.resolve_approved_video_release_artifact($1,$2)", [seeded.workflowId, seeded.runId])).includes("NOT_FOUND"));
  const unapproved = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_RELEASE", release, "validate-approved-packaging", provenance, { approve: false, finalizerStep: "finalize-video-release", finalQaStep: "final-video-release-qa" });
  record("unapproved release is rejected", (await expectFailure(db, ownerA, "select channelwright.resolve_approved_video_release_artifact($1,$2)", [unapproved.workflowId, unapproved.runId])).includes("UPSTREAM_RELEASE_NOT_APPROVED"));
  const corrupt = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_RELEASE", release, "validate-approved-packaging", provenance, { corruptHash: true, finalizerStep: "finalize-video-release", finalQaStep: "final-video-release-qa" });
  record("altered release hash fails integrity", (await expectFailure(db, ownerA, "select channelwright.resolve_approved_video_release_artifact($1,$2)", [corrupt.workflowId, corrupt.runId])).includes("UPSTREAM_RELEASE_INTEGRITY_MISMATCH"));
  ...
  record("approved VIDEO_RELEASE run payload cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_runs set output_payload = output_payload || '{\"x\":1}'::jsonb where id=$1", [seeded.runId])).includes("APPROVED_ARTIFACT_IMMUTABLE"));

  const perfSeed = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_PERFORMANCE", { schemaVersion: 1 }, "validate-approved-release", { reference: {} }, { finalizerStep: "finalize-video-performance", finalQaStep: "final-video-performance-qa" });
  record("approved VIDEO_PERFORMANCE run payload cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_runs set output_payload = output_payload || '{\"x\":1}'::jsonb where id=$1", [perfSeed.runId])).includes("APPROVED_ARTIFACT_IMMUTABLE"));
  record("approved VIDEO_PERFORMANCE finalized step output cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_steps set output_payload = output_payload || '{\"x\":1}'::jsonb where workflow_run_id=$1 and step_key='finalize-video-performance'", [perfSeed.runId])).includes("APPROVED_ARTIFACT_IMMUTABLE"));
  ...
  const perfWorkflow = perfSeed.workflowId;
  const releaseRunId = seeded.runId;
  const insertPerf = () => db.query(`insert into channelwright.workflow_runs(id,owner_id,workflow_id,workflow_type,definition_version,status,idempotency_key,input_hash,input_payload)
    values ($1,$2,$3,'CHANNEL_VIDEO_PERFORMANCE',1,'QUEUED',$4,$5,$6::jsonb)`,
    [randomUUID(), ownerA, perfWorkflow, `disp-conc:${randomUUID()}`, "1".repeat(64), JSON.stringify({ approvedVideoReleaseReference: { releaseRunId } })]);
  await insertPerf();
  let blocked = false;
  try { await insertPerf(); } catch { blocked = true; }
  record("second active VIDEO_PERFORMANCE for same owner+release blocked by unique index", blocked);
  ...
  const nonZero = await expectFailure(db, null, "select channelwright.ensure_research_run_budget($1,$2,$3,$4,$5::jsonb)", [perfSeed.runId, randomUUID(), randomUUID(), "g", limits(4)]);
  record("nonzero-retrieval VIDEO_PERFORMANCE budget rejected by VALIDATION_ERROR", nonZero.includes("VALIDATION_ERROR") && !nonZero.includes("LEASE_NOT_ACTIVE"), nonZero.split("\n")[0].slice(0, 160));
  const zero = await expectFailure(db, null, "select channelwright.ensure_research_run_budget($1,$2,$3,$4,$5::jsonb)", [perfSeed.runId, randomUUID(), randomUUID(), "g", limits(0)]);
  record("zero-retrieval VIDEO_PERFORMANCE budget passes ceilings (fails only on lease)", zero.includes("LEASE_NOT_ACTIVE") && !zero.includes("VALIDATION_ERROR"), zero.split("\n")[0].slice(0, 160));
}
...
async function main() {
  if (!CONNECTION) {
    console.log(JSON.stringify({ gate: "VIDEO_PERFORMANCE_DISPOSABLE_PG_UNAVAILABLE", reason: "CHANNELWRIGHT_DISPOSABLE_DATABASE_URL is not set", hint: "Set CHANNELWRIGHT_DISPOSABLE_DATABASE_URL to a throwaway PostgreSQL cluster (never the application database)." }, null, 2));
    process.exitCode = 2;
    return;
  }
  ...
  try { await admin.connect(); }
    ...
    console.log(JSON.stringify({ gate: "VIDEO_PERFORMANCE_DISPOSABLE_PG_UNAVAILABLE", reason: error instanceof Error ? error.message.split("\n")[0] : String(error), hint: "Set CHANNELWRIGHT_DISPOSABLE_DATABASE_URL to a reachable throwaway PostgreSQL cluster." }, null, 2));
    process.exitCode = 2;
    return;
  }
  ...
  console.log(JSON.stringify({
    gate: failed.length === 0 ? "VIDEO_PERFORMANCE_DISPOSABLE_PG_PASSED" : "VIDEO_PERFORMANCE_DISPOSABLE_PG_FAILED",
