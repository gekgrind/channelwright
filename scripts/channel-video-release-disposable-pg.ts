/**
 * Disposable PostgreSQL runtime gate for the CHANNEL_VIDEO_RELEASE migration
 * chain.
 *
 ...
 * VIDEO_RELEASE coverage, concurrency, canonical parity, selectable-candidate
...
async function runChecks(db: pg.Client) {
  const packaging = approvedVideoPackagingArtifactFixture.packagingResult;
  const discovery = approvedVideoPackagingArtifactFixture.discoveryBundle;
  const scriptRef = packaging.upstreamVideoScript;
  // The packaging's provenance step output (validate-approved-script) carries the
  // exact upstream script reference and the discovery evidence bundle.
  const provenance = { reference: scriptRef, scriptResult: {}, discoveryBundle: discovery, scope: {} };
  ...
  const fns = await db.query<{ count: string }>(
    "select count(*)::text as count from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='channelwright' and p.proname in ('resolve_approved_research_artifact','resolve_approved_strategy_artifact','resolve_approved_content_artifact','resolve_approved_video_brief_artifact','resolve_approved_video_script_artifact','resolve_approved_video_packaging_artifact')");
  record("all six approved-artifact resolvers compiled", fns.rows[0].count === "6", `count=${fns.rows[0].count}`);

  // --- canonical parity --------------------------------------------------
  const contract = packaging.viewerValue.contract;
  ...
  // --- resolver invalid --------------------------------------------------
  record("cross-owner resolution is NOT_FOUND", (await expectFailure(db, ownerB, "select channelwright.resolve_approved_video_packaging_artifact($1,$2)", [seeded.workflowId, seeded.runId])).includes("NOT_FOUND"));
  const unapproved = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_PACKAGING", packaging, "validate-approved-script", provenance, { approve: false, finalizerStep: "finalize-video-packaging", finalQaStep: "final-video-packaging-qa" });
  record("unapproved packaging is rejected", (await expectFailure(db, ownerA, "select channelwright.resolve_approved_video_packaging_artifact($1,$2)", [unapproved.workflowId, unapproved.runId])).includes("UPSTREAM_PACKAGING_NOT_APPROVED"));
  const corrupt = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_PACKAGING", packaging, "validate-approved-script", provenance, { corruptHash: true, finalizerStep: "finalize-video-packaging", finalQaStep: "final-video-packaging-qa" });
  record("altered packaging hash fails integrity", (await expectFailure(db, ownerA, "select channelwright.resolve_approved_video_packaging_artifact($1,$2)", [corrupt.workflowId, corrupt.runId])).includes("UPSTREAM_PACKAGING_INTEGRITY_MISMATCH"));
  ...
  const packagingRun = seeded.runId;
  record("approved VIDEO_PACKAGING run payload cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_runs set output_payload = output_payload || '{\"x\":1}'::jsonb where id=$1", [packagingRun])).includes("APPROVED_ARTIFACT_IMMUTABLE"));

  const releaseSeed = await seedApprovedRun(db, ownerA, "CHANNEL_VIDEO_RELEASE", { schemaVersion: 1 }, "validate-approved-packaging", { reference: {} }, { finalizerStep: "finalize-video-release", finalQaStep: "final-video-release-qa" });
  record("approved VIDEO_RELEASE run payload cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_runs set output_payload = output_payload || '{\"x\":1}'::jsonb where id=$1", [releaseSeed.runId])).includes("APPROVED_ARTIFACT_IMMUTABLE"));
  record("approved VIDEO_RELEASE finalized step output cannot be rewritten", (await expectFailure(db, null, "update channelwright.workflow_steps set output_payload = output_payload || '{\"x\":1}'::jsonb where workflow_run_id=$1 and step_key='finalize-video-release'", [releaseSeed.runId])).includes("APPROVED_ARTIFACT_IMMUTABLE"));
  ...
  const releaseWorkflow = releaseSeed.workflowId;
  const insertRelease = () => db.query(`insert into channelwright.workflow_runs(id,owner_id,workflow_id,workflow_type,definition_version,status,idempotency_key,input_hash,input_payload)
    values ($1,$2,$3,'CHANNEL_VIDEO_RELEASE',1,'QUEUED',$4,$5,$6::jsonb)`,
    [randomUUID(), ownerA, releaseWorkflow, `disp-conc:${randomUUID()}`, "1".repeat(64), JSON.stringify({ approvedVideoPackagingReference: { packagingRunId: packagingRun } })]);
  await insertRelease();
  let blocked = false;
  try { await insertRelease(); } catch { blocked = true; }
  record("second active VIDEO_RELEASE for same owner+packaging blocked by unique index", blocked);
  ...
  const nonZero = await expectFailure(db, null, "select channelwright.ensure_research_run_budget($1,$2,$3,$4,$5::jsonb)", [releaseSeed.runId, randomUUID(), randomUUID(), "g", limits(4)]);
  record("nonzero-retrieval VIDEO_RELEASE budget rejected by VALIDATION_ERROR", nonZero.includes("VALIDATION_ERROR") && !nonZero.includes("LEASE_NOT_ACTIVE"), nonZero.split("\n")[0].slice(0, 160));
  const zero = await expectFailure(db, null, "select channelwright.ensure_research_run_budget($1,$2,$3,$4,$5::jsonb)", [releaseSeed.runId, randomUUID(), randomUUID(), "g", limits(0)]);
  record("zero-retrieval VIDEO_RELEASE budget passes ceilings (fails only on lease)", zero.includes("LEASE_NOT_ACTIVE") && !zero.includes("VALIDATION_ERROR"), zero.split("\n")[0].slice(0, 160));
}
...
async function main() {
  if (!CONNECTION) {
    console.log(JSON.stringify({ gate: "VIDEO_RELEASE_DISPOSABLE_PG_UNAVAILABLE", reason: "CHANNELWRIGHT_DISPOSABLE_DATABASE_URL is not set", hint: "Set CHANNELWRIGHT_DISPOSABLE_DATABASE_URL to a throwaway PostgreSQL cluster (never the application database)." }, null, 2));
    process.exitCode = 2;
    return;
  }
  ...
  try { await admin.connect(); }
    ...
    console.log(JSON.stringify({ gate: "VIDEO_RELEASE_DISPOSABLE_PG_UNAVAILABLE", reason: error instanceof Error ? error.message.split("\n")[0] : String(error), hint: "Set CHANNELWRIGHT_DISPOSABLE_DATABASE_URL to a reachable throwaway PostgreSQL cluster." }, null, 2));
    process.exitCode = 2;
    return;
  }
  ...
  console.log(JSON.stringify({
    gate: failed.length === 0 ? "VIDEO_RELEASE_DISPOSABLE_PG_PASSED" : "VIDEO_RELEASE_DISPOSABLE_PG_FAILED",
