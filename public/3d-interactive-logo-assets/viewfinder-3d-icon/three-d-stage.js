...
(() => {
  const stylesheet = `
    :host {
      ...
      width: 100%;
      height: 100vh;
      background: var(--stage-bg, #0b1524);
      overflow: hidden;
    }
    ...
    .toolbar button:hover { background: rgba(0,212,255,0.14); border-color: rgba(0,212,255,0.5); }
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      const softbox = (w, h, d, color, intensity, pos) => {
      ...
      softbox(30, 30, 1, 0x0a1420, 1.0, [0, 0, -14]);
      softbox(30, 1, 30, 0x08111c, 1.0, [0, -12, 0]);
      softbox(12, 8, 0.2, 0xffffff, 3.4, [-6, 7, 7]);
      softbox(9, 11, 0.2, 0xcfe6ff, 2.2, [8, 2, 4]);
      softbox(0.3, 8, 9, 0x00d4ff, 2.2, [-9, 1, -3]);
      softbox(6, 0.4, 6, 0xd27a2c, 0.9, [4, -5, 5]);
      const pmrem = new THREE.PMREMGenerator(renderer);
      scene.environment = pmrem.fromScene(envScene, 0.04).texture;
      scene.environmentIntensity = 2.1;
      pmrem.dispose();

      scene.add(new THREE.HemisphereLight(0x8fb7d6, 0x0a1018, 0.35));
    ...
    setObject(object) {
      ...
      if (!box.isEmpty()) {
        ...
        const dir = new THREE.Vector3(1.15, 0.42, 1.3).normalize();
