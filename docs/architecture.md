...
Each workflow type declares one canonical finalizer whose output becomes the run's durable `output_payload`: `synthesize-validation` for `CHANNEL_CONCEPT_VALIDATION` and `CHANNEL_RESEARCH`, `finalize-strategy` for `CHANNEL_STRATEGY`, `finalize-content-intelligence` for `CHANNEL_CONTENT_INTELLIGENCE`, `finalize-video-brief` for `CHANNEL_VIDEO_BRIEF`. Promotion is keyed on the run's type, so intermediate steps never become the final artifact. Paid workflows are additionally rate-bounded in the database: one active research run per owner, one active strategy run per owner and approved research run, one active content-intelligence run per owner and approved strategy run, and one active video-brief run per owner, approved content run, and selected topic.

## Evidence chain and the Viewer Value invariant

The four provider-backed workflows form one provenance chain. `CHANNEL_RESEARCH` establishes whether a channel opportunity is real; `CHANNEL_STRATEGY` consumes one exact approved research artifact and decides how the channel competes; `CHANNEL_CONTENT_INTELLIGENCE` consumes one exact approved strategy artifact and decides what to make next; `CHANNEL_VIDEO_BRIEF` consumes one exact approved content-intelligence artifact plus one eligible topic and decides what that single video must accomplish and how production should deliver it (see `docs/video-brief.md`). Each downstream workflow resolves its upstream artifact through a database-authoritative RPC, persists an immutable reference, and re-verifies that reference before any model use. Each reference transitively carries its own upstream reference, so the whole research -> strategy -> content -> brief chain remains queryable from the final artifact.
