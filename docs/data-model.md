...
`202608150001_video_brief.sql` adds the fourth paid workflow type, `CHANNEL_VIDEO_BRIEF`, additively. It introduces `resolve_approved_content_artifact` (which also resolves and verifies the selected backlog topic), `canonical_jsonb_text` for key-order-independent contract hashing, and the partial unique index `workflow_runs_active_video_brief_uniq` keyed on owner, approved content run, and selected topic. It performs no external retrieval, so it is required to declare zero provider requests, quota units, and searches; no database ceiling is widened.
