...
Four paid-run limits are enforced in PostgreSQL rather than the browser. `RESEARCH_LIMIT_REACHED` rejects a second active `CHANNEL_RESEARCH` run per owner. `STRATEGY_LIMIT_REACHED` rejects a second active `CHANNEL_STRATEGY` run per owner and approved upstream research run, backed by `workflow_runs_active_strategy_uniq`. `CONTENT_LIMIT_REACHED` rejects a second active `CHANNEL_CONTENT_INTELLIGENCE` run per owner and approved strategy run, backed by `workflow_runs_active_content_uniq`. `VIDEO_BRIEF_LIMIT_REACHED` rejects a second active `CHANNEL_VIDEO_BRIEF` run per owner, approved content run, and selected topic, backed by `workflow_runs_active_video_brief_uniq`; two different topics from one backlog may be briefed concurrently. All four surface as HTTP 429. Human-revision successors and regeneration after a terminal run remain permitted.
...
`CHANNEL_VIDEO_BRIEF` is the opposite case: it performs no external retrieval at all and reasons over the discovery bundle the approved content run already paid for, so its search, provider-request, and quota ceilings are exactly zero and only model calls consume budget. Monitor synthesis, critic, QA, and revision calls and token spend. Budget exhaustion is terminal.
