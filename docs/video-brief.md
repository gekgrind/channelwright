...
One pre-existing defect outside this chain remains open: `channelwright.execute_media_production_action` calls `digest()` under the same narrow search_path and will fail identically at runtime. It belongs to the media-production pipeline, not the workflow chain, so it was documented rather than changed here. The fix is the same one-line `ALTER FUNCTION`.
